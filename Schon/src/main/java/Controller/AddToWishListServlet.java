...
public class AddToWishListServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(!AddToWishListDAO.checkCustomerID(customer.getCustomerID())) {
		...
		if(!AddToWishListDAO.checkProductID(productID)) {
			response.getWriter().write(new Gson().toJson(new JsonResult("ID sản phẩm không tồn tại!", 0)));
			return;
		}
		
		if(AddToWishListDAO.checkWishList(customer.getCustomerID(), productID)) {
			response.getWriter().write(new Gson().toJson(new JsonResult("Sản phẩm đã tồn tại trong danh sách theo dõi!", 0)));
			return;
		}
		
		WishList wishList = new WishList(customer.getCustomerID(), productID);
		AddToWishListDAO.createWishList(wishList);
		
		int wishListCount = AddToWishListDAO.getWishListCount(customer.getCustomerID());
		
		response.getWriter().write(new Gson().toJson(new JsonResult("Thêm vào danh sách theo dõi thành công!", wishListCount)));
	}
